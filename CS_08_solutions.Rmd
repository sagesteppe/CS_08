...
```{r View Data}
data(iris)
knitr::kable(head(iris), booktab = T)
```

We will now create a table of the summary statistics of this species here. 

```{r Create Iris Summary Statistics}

a <- iris %>%  # use map and modify the outputs
  split(.$Species) %>% 
  map(summary) %>% 
  map(as.data.frame) %>% 
  map(drop_na) %>% 
...
iris_table <- iris %>% # use summarize
  ...
  # here we work on styling the values for the table
...
knitr::kable(model_summary_statistics, 
             ...
             align = "cccc",
             caption = "Results of a linear model with one continuous predictor (Sepal Length) and response (Sepal Width) displays an insignifcant negative relationship",
             booktabs = T
             ) %>% 
  kable_classic(full_width = F, html_font = "Cambria")
...
We can very simply plot our linear model using the intercept and slope. 
...
ggplot(conf_int, aes(x = Sepal.Width, y = Sepal.Length))+
  ...
  # we will add the p-value and r^2 to the plot - it i  s a little busy for my liking !
...
rm(model, conf_int, pred_int)
```


```{r Calculate our own prediction via this method, eval = F}
...
```{r Plot relationship between single continuous predictor and response with a confidence interval -Compare results, eval= F}
ggplot(iris, aes(x = Sepal.Width, y = Sepal.Length))+
  geom_jitter()+
